...
start_result = int(input('Введите результат спортсмена в первый день: '))
finish_result = int(input('Ведите результат которого хочет достичь спортсмен: '))
day = 1
print('Результат:')
print(f'{day}-й день: {start_result}')
while start_result <= finish_result:
    start_result += start_result * 0.1
    day += 1
    print(f'{day}-й день: {start_result:.2f}')
print(f'Ответ: на {day}-й день спортсмен достиг результата — не менее {finish_result} км.')
